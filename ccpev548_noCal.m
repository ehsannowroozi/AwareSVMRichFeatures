function F = ccpev548_noCal(FILE)
% -------------------------------------------------------------------------
...
% [FILE_REF] = create_reference_image(X,QF);  % COSA FA QUESTA FUNZIONE
...
%  % compute DCT (not only jpg)
% %a = imread(path);
% a = path;
% fun = @dct2;
% Plane = blkproc(a,[8 8],fun);
